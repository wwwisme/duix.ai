...
- (instancetype _Nullable)initWithSampleRate:(NSInteger)fs nsMode:(BZQNSMode)mode{
    if (self = [super init]) {
        ...
        int status = GJLWebRtcNs_Init(_handle, (uint32_t)fs);
        if (status != 0) {
            NSLog(@"WebRTC初始化失败！%d", status);
            return nil;
        }

        status = GJLWebRtcNs_set_policy(_handle, (int)mode);
        if (status != 0) {
...
//    }
    ...
    for(int i = 0; i < sTot; ++i) {
        ...
        GJLWebRtcNs_Analyze(_handle, nsIn[0]);
        GJLWebRtcNs_Process(_handle, (const short *const *)nsIn, 1, nsOut);
